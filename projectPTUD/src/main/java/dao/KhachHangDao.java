...
//			tr.begin();
	...
	public List<NhanVien> DanhsachKhachHang(){
		...
		try {
			...
			List<NhanVien> dsKhachHang = session.createNativeQuery(sql, NhanVien.class).getResultList();
...
//	String query ="SELECT *FROM KhachHang where  hoTen like '%" + tenKH + " %' and gioiTinh like '%" + gioiTinh + " %' order by maKH desc OFFSET " + offset + " ROWS FETCH NEXT " + limit + " ROWS ONLY;";
			String sql = "SELECT *FROM KhachHang  where hoTen like '%" + tenKH + "%' and gioiTinh like '%" + gioiTinh
					+ "%' order by maKH desc OFFSET " + offset + "  ROWS FETCH NEXT " + limit + "  ROWS ONLY;";
	...
	public int tongTrang(String txtSearch, String gioiTinh, int limit) {
		...
		try {
			tr.begin();
			String sql = "select count(*) from KhachHang where hoTen like N'%" + txtSearch + "%'   and gioiTinh like '%"
					+ gioiTinh + "%' ";
