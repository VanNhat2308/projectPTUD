...
//			JOptionPane.showMessageDialog(this, "Không có phiếu đặt phòng");
		...
		for (PhieuDatPhong p : dsPhieuDatPhong) {
			tableModel.addRow(new String[] { p.getMaPDP().substring(3), p.getPhong().getMaPhong(),
					p.getKhachHang().getSoDienThoai(), FormatCustom.dinhDanhThoiGian(p.getNgayDKDatPhong()),
					FormatCustom.dinhDanhThoiGian(p.getNgayNhanPhong()),
...
//			"SĐT Khách", "Thời gian lập phiếu", "Thời gian nhận phòng", "Trạng thái" }));
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (object == btnHuyPhong) {
			...
			if (phieuDatPhong.getPhong().getTrangThaiPhong().equals(MainFrame.maPhongTam)) {
...
//			ChiTietPhongDiaglog chiTietPhongDiaglog = new ChiTietPhongDiaglog();
//			ChiTietPhongDiaglog.kho
//			return;
		}
