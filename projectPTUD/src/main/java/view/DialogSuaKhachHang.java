...
//	}
	...
	public DialogSuaKhachHang(String ma) {
		...
		cbGioiTinh.setBounds(481, 96, 189, 35);
		...
		textTenKH.setBounds(170, 155, 634, 35);
		...
		btnCapNhat.setBounds(675, 412, 131, 40);
	...
	private void loadDuLieu(KhachHang khachHang) {
		...
		textEmail.setText(khachHang.getEmail().strip());
		textCMND.setText(khachHang.getSoCMND().strip());
	}
	...
	public boolean themKH() {
		...
		String email = textEmail.getText().strip();
		if (email.isEmpty()) {
		...
		String CMND =textCMND.getText().strip();
		if (CMND.isEmpty()) {
