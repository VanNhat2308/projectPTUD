...
public class DanhSachPhongPanel extends JPanel implements ActionListener, ItemListener, MouseListener, KeyListener {
	...
	private ChiTietPhongDiaglog chiTietPhongDiaglog;
	private JButton btnDatTheoDoan;
		...
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					...
					DanhSachPhongPanel frame = new DanhSachPhongPanel();
	...
	public DanhSachPhongPanel() {
		...
		contentPane.setBounds(0, 11, 1555, 762);
		contentPane.setBorder(new LineBorder(Color.LIGHT_GRAY, 1, true));

		panelDanhSachPhong = new JPanel();
//		panelDanhSachPhong.addAncestorListener(new AncestorListener() {
...
//				.addComponent(lbPhongVuaVao, GroupLayout.PREFERRED_SIZE, 361, GroupLayout.PREFERRED_SIZE).addGap(18)));
		gl_panel_2.setVerticalGroup(gl_panel_2.createParallelGroup(Alignment.TRAILING).addGroup(
...
//						.addGap(2)));
		...
		btnDatTheoDoan = new JButton("Đặt Theo Đoàn        (F12)");
		btnDatTheoDoan.setHorizontalAlignment(SwingConstants.LEFT);
		btnDatTheoDoan.setForeground(Color.BLACK);
		btnDatTheoDoan.setFont(new Font("Tahoma", Font.BOLD, 15));
		btnDatTheoDoan.setBackground(Color.ORANGE);
		clickOnKey(btnDatTheoDoan, "dattheodoan", KeyEvent.VK_F11);
		btnDatTheoDoan.addActionListener(this);
		...
		gl_panel.setHorizontalGroup(gl_panel.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_panel.createSequentialGroup().addGap(9)
						.addGroup(gl_panel.createParallelGroup(Alignment.LEADING)
								...
								.addComponent(btnDatTheoDoan, GroupLayout.DEFAULT_SIZE, 212, Short.MAX_VALUE)
		...
		gl_panel.setVerticalGroup(gl_panel.createParallelGroup(Alignment.LEADING)
				.addGroup(gl_panel.createSequentialGroup().addGap(8)
						...
						.addComponent(btnDatTheoDoan, GroupLayout.DEFAULT_SIZE, 48, Short.MAX_VALUE).addGap(13)
		...
		gl_contentPane.setHorizontalGroup(gl_contentPane.createParallelGroup(Alignment.LEADING)
				...
				.addGroup(gl_contentPane.createSequentialGroup().addGap(9)
						.addComponent(
								panel, GroupLayout.PREFERRED_SIZE, GroupLayout.DEFAULT_SIZE, GroupLayout.PREFERRED_SIZE)
						.addGap(10)
						.addGroup(gl_contentPane.createParallelGroup(Alignment.LEADING)
								.addComponent(panel_1, GroupLayout.DEFAULT_SIZE, 1291, Short.MAX_VALUE)
								.addComponent(scrollPane, GroupLayout.DEFAULT_SIZE, 1291, Short.MAX_VALUE))
						.addContainerGap()));
		gl_contentPane.setVerticalGroup(gl_contentPane.createParallelGroup(Alignment.LEADING).addGroup(gl_contentPane
				.createSequentialGroup()
				.addComponent(lblNewLabel_4, GroupLayout.PREFERRED_SIZE, 50, GroupLayout.PREFERRED_SIZE).addGap(18)
		...
		chiTietPhongDiaglog = new ChiTietPhongDiaglog();
...
//			
		...
		for (int i = 0; i < length; i++) {
			...
			if (phong.getTrangThaiPhong().equals(MainFrame.maPhongCho)
					|| phong.getTrangThaiPhong().equals(MainFrame.maPhongTam)) {
				try {
					PhieuDatPhong phieuDatPhong = phieuDatPhongDao.layPhieuDatPhongMoiNhatTheoPhong(phong.getMaPhong());
					Date dateAdd2Hours = phieuDatPhong.getNgayNhanPhong();
	...
	public void actionPerformed(ActionEvent e) {
		...
		if (o == btnDatTheoDoan) {
			new DialogDatTheoDoan(null, null);
		}
		else if (o == btnTim) {
		...
		// phan duoi
			...
			if (o == btnDatPhongCho) {
				DialogDatPhongTruoc phieuDatPhongCho = new DialogDatPhongTruoc(phong);
				...
				hoaDon.setChiTietHoaDon(Arrays.asList(new ChiTietHoaDon(phong, hoaDon, null)));
				phongDao.capNhatTrangThaiPhong(phong.getMaPhong(), MainFrame.maPhongBan);
				loadGUIDanhSachPhong(null, 1, 0, 0);
				phieuDatPhongDao.huyPhieuDatPhong(phieuDatPhong.getMaPDP());
				...
				if (phong.getTrangThaiPhong().equalsIgnoreCase(MainFrame.maPhongBan)) {
					JOptionPane.showConfirmDialog(null, "Vui lòng chọn phòng trống", "Thông báo",
				...
				Date ngayNhap = hoaDon.getNgayTraPhong();
				Date now = new Date();
				boolean k = true;
				...
				if (hoaDon != null && k == true) {
					loadGUIDanhSachPhong(null, 1, 0, 0);
					JOptionPane.showConfirmDialog(null, "Đặt phòng "+phong.getMaPhong().substring(1)+" thành công", "Thông báo",
							JOptionPane.DEFAULT_OPTION);
...
//					}
				...
				if (phong.getTrangThaiPhong().equals(MainFrame.maPhongTam)) {
					phong.setTrangThaiPhong("pc007");
					phongDao.capNhatTrangThaiPhong(phong.getMaPhong(), MainFrame.maPhongBan);
				} else {
					phong.setTrangThaiPhong("TTP003");
					phongDao.capNhatTrangThaiPhong(phong.getMaPhong(), MainFrame.maPhongTrong);
			   ...
			   chiTietPhongDiaglog.khoiTao(phong.getMaPhong().strip());
		...
		if (e.getButton() == MouseEvent.BUTTON1) {
			...
			if (listHashMap.get(o) != null) {
				...
				if (e.getClickCount() == 2 && e.getButton() == MouseEvent.BUTTON1) {
					chiTietPhongDiaglog.khoiTao(phong.getMaPhong());
				}
	...
	private void disabledPhongDangCho(Phong phong) {
		disabledTheoDSPhong();
		if (phong.getTrangThaiPhong().strip().equals(MainFrame.maPhongTrong)) {
			btnNhanPhong.setEnabled(false);
			btnHuyPhongCho.setEnabled(false);
			btnDatPhongNgay.setEnabled(true);
			btnDatPhongCho.setEnabled(true);
		} else if (phong.getTrangThaiPhong().strip().equals(MainFrame.maPhongCho)) {
			btnNhanPhong.setEnabled(true);
			btnHuyPhongCho.setEnabled(true);
			btnDatPhongCho.setEnabled(false);
			btnDatPhongNgay.setEnabled(true);
		} else if (phong.getTrangThaiPhong().equals(MainFrame.maPhongTam)) {
			btnHuyPhongCho.setEnabled(true);
		} else {
			btnTinhTien.setEnabled(true);
			btnChuyenPhong.setEnabled(true);
...
//			_timeField.setText("" + hh + ":" + mm + ":" + ss);
	...
	public void setPanel(JPanel parent) {
		...
		parent.add(lblTt);
		JLabel lblNewLabel_5 = new JLabel(covertTrangThai(phong.getTrangThaiPhong().strip()));
		...
		if (phong.getTrangThaiPhong().equals(MainFrame.maPhongBan)
				|| phong.getTrangThaiPhong().equals(MainFrame.maPhongTam)) {
			HoaDon hoaDon = hoaDonDao.layHoaDonMoiNhatTheoPhong(phong.getMaPhong());
